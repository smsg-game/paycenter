          ...
          <li style="padding: 1.5em 15px;">
            ...
            <div data-role="popup" id="gcard-amount-popup" data-overlay-theme="a">
              <fieldset data-role="controlgroup" id="fieldset-for-gcard-type-6">
                ...
                <a href="" data-role="button" money="25">25元</a>
                <a href="" data-role="button" money="30">30元</a>
                <a href="" data-role="button" money="35">35元</a>
              ...
              <fieldset data-role="controlgroup" id="fieldset-for-gcard-type-8" style="display:none">
                 ...
                 <a href="" data-role="button" money="50">50元</a>
                 <a href="" data-role="button" money="100">100元</a>
                 <a href="" data-role="button" money="250">250元</a>
...
2、选择充值卡面值时，请与您的卡实际面值保持一致，以免造成充值失败和充值卡失效。
  ...
  <script>
    $(function() {
    	...
    	var input = $("#gcard-no");
    	var password = $("#gcard-pwd");
    	var ok = $("#ok");
    	var eb = $("#eb");
    	//卡类型
      var $gcardType = $('input[name=gcard-type]');
      var $gcardTypePopup = $('#gcard-type-popup');
      var $gcardTypeAcceptor = $('#gcard-type-acceptor');
      
      //卡金额选择相关
      var $gcardAmount = $('input[name=gcard-amount]');
      var $gcardAmountPopup = $('#gcard-amount-popup');
      var $gcardAmountAcceptor = $('#gcard-amount-acceptor');
      var $gcardAmountPopupFieldsets = $('#gcard-amount-popup > fieldset');

      var c = $gcardType.val(), m = $gcardAmount.val();
      
      $gcardTypeAcceptor.click(function() {
        $gcardTypePopup.popup('open');
      });
      $gcardTypePopup.find('a').click(function() {
        var $this = $(this);

        $gcardTypeAcceptor.text('点卡类型：' + $this.text());
        $gcardTypePopup.popup('close');
        c = $this.attr("gcardType");
        
        // Used for change different value for different card.
        $gcardAmountPopupFieldsets.css("display", "none");
        $('#fieldset-for-gcard-type-' + c).css("display", "block").find('a').first().click();
        
        $gcardType.val(c);
      });

      $gcardAmountAcceptor.click(function() {
        $gcardAmountPopup.popup('open');
      });
      $gcardAmountPopup.find('a').click(function() {
        var $this = $(this);

        $gcardAmountAcceptor.html('点卡面额：<span class="font-striking">' + parseInt($this.text()) + '</span>元');
        $gcardAmountPopup.popup('close');
        m = $this.attr("money");
        $gcardAmount.val(m);
