...
<% String url = request.getScheme() + "://" + request.getServerName()+ request.getContextPath(); 
          url = url +"/mobile/return!returnResult.e";
%>
...
$(document).ready(function(){
	...
	$("#ok").live("click touchStart",function(){
	    if(verify()){
	       $.getJSON("/mobile/smsYhxf!smsCharge.e?money="+money.val()+"&phoneNum="+phoneNum.val(),function(data) {
	             var invoice = data.invoice;
			    if(data.status=='success'){
			       window.AndroidSmsPayForJavaScript.startSmsPay(data.msg,data.number,'<%=url%>?invoice='+invoice);
			    }else{
			       location.href = "/mobile/redirect!showResult.e?status="+data.status+"&msg="+encodeURIComponent(data.msg)+"&invoice="+data.invoice;
			    }
		   }); 
	    }
	})
