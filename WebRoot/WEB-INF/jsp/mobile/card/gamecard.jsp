...
<div class="mode">
	<div class="tel-mode"><div id="menu">
		...
		<div class="tel-money">
			...
			<h6>温馨提示：</h6>
			<div>
			<p  id="tit-6" class="gayText">1、如果支付金额大于订单金额，多付的钱会自动充值到您账号的e币；<br/>2、请使用卡号以CSC5、CS、S、CA、CSB、YA、YB、YC、YD、80133开头的"盛大互动娱乐卡"进行支付；<br />3、选择充值卡面值时，请与您的卡实际面值保持一致，以免造成充值失败和充值卡失效。</p>
			<p  id="tit-8" style="display:none" class="gayText">1、如果支付金额大于订单金额，多付的钱会自动充值到您账号的e币；<br/>2、全国官方征途游戏充值卡可用；<br />3、选择充值卡面值时，请与您的卡实际面值保持一致，以免造成充值失败和充值卡失效。</p>
			<p  id="tit-7" style="display:none"  class="gayText">1、如果支付金额大于订单金额，多付的钱会自动充值到您账号的e币；<br/>2、不能使用特定游戏专属充值卡支付。 特定游戏包括大唐风云、传说、蜗牛、猫扑一卡通、九鼎、雅典娜、山河等游戏。<br />3、在此使用过的骏网一卡通，卡内剩余J点只能在易宝支付合作商家进行支付使用。</p>
			<p  id="tit-12" style="display:none"  class="gayText">1、如果支付金额大于订单金额，多付的钱会自动充值到您账号的e币；<br/>2、全国官方完美游戏充值卡可用；<br />3、选择充值卡面值时，请与您的卡实际面值保持一致，以免造成充值失败和充值卡失效。</p>
			</div>
...
$(document).ready(function(){
	...
	company.live("click touchStart",function(){
		...
		<%if(Constants.isTest){%>
				if(n==0){   //盛大卡
					$("#input").val("S110E0000604584");
					$("#password").val("55780457");
				}else if(n==1){//征途卡
					$("#input").val("0100001206785051");
					$("#password").val("70925644");
				}else if(n==2){  //骏网卡
					$("#input").val("1111151124743127");
					$("#password").val("9436143058405327");
				}else if(n==3){ //完美卡
					$("#input").val("6017299921");
					$("#password").val("460191692488773");
				}
		<%}%>
	...
	ok.live("click touchStart",function(){
		...
		if(verifyCard() && passwordCard()){
			...
			//相等
				...
				var successUrl= "/mobile/redirect!showResult.e"; //支付成功，导向到此；
