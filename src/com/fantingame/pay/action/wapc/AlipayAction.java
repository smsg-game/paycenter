...
public class AlipayAction extends BaseAction {
	...
	public String execute(){
		try {
			PayTrade trade = getTradeByInvoice(invoice); //获取订单
			if(trade == null) throw new EasouPayException(TradeCode.EASOU_CODE_140.code,TradeCode.EASOU_CODE_140.msgC);
			PayEb payEb = newPayEbOrder(invoice,Constants.CHANNEL_ID_ALIPAY_WAP,trade.getReqFee(),null); //下订单
			PayChannel channel = getChannelById(Constants.CHANNEL_ID_ALIPAY_WAP);	//获取支付商信息
			
			Map<String, String> params = prepareAliTradeParamMap(trade,payEb,channel);  //组装参数
	...
	public String aliCharge(){
		try {
			...
			PayTrade trade = newChargeTrade(money,backUrl,"wapc",invoiceId);
	...
	private  Map<String, String> prepareCommonParamMap(PayChannel channel){
        ...
        params.put("call_back_url", channel.getReturnUrl());
